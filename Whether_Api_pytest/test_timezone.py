...
URL="http://api.weatherapi.com/v1/timezone.json?key=a95fc31d83624cdfbda52019212207&q=London&dt=2021-07-22"
response = requests.get(URL)
data = response.json()
print(data)
#POSITIVE TETCASES
#Positive Testcase For Testing The Status Code of the timezone Api
def test_timezone_status_code():
    assert response.status_code==200
    logging.info("status code  is 200.successfully passsed positive testcase")

#Positive Testcase For Testing The type of content of the Timezone   Api
def test_timezone_content_type_equals_to_json():
    assert response.headers["Content-Type"] == "application/json"
    logging.info("content type is application/json .successfully passsed positive testcase")

#Positive Testcase For Testing The tz-id of the Timezone Api
def test_timezone_tz_id():
    assert data["location"]["tz_id"]=="Europe/London"
    logging.info("tz-id is Europe/London .successfully passsed positive testcase")

#Positive Testcase For Testing The Country of the Timezone Api
def test_timezone_country():
    assert data['location']['country']=='United Kingdom'
    logging.info("country is United Kingdom.successfully passsed positive testcase")

#Positive Testcase For Testing The Type of Response of the Timezone Api
...
def test_negative_timezone_status_code():
    assert response.status_code!=400
    logging.info("status code  is 400.successfully passsed negative testcase")

#Negative Testcase For Testing The type of content of the Timezone Api
def test_negative_timezone_content_type_equals_to_json():
    assert response.headers["Content-Type"] != "application/xml"
    logging.info("content type is application/xml .successfully passsed negative testcase")

#Negative Testcase For Testing The tz-id of the Timezone Api
def test_negative_timezone_tz_id():
    assert data["location"]["tz_id"]!="Europe/russia"
    logging.info("tz-id is europe/russia .successfully passsed negative testcase")

#Negative Testcase For Testing The Country of the Timezone Api
def test_negative_timezone_country():
    assert data['location']['country']!='United states'
    logging.info("Country is United states .successfully passsed negative testcase")

#Negative Testcase For Testing The Type of Response of the Timezone Api
def test_negative_timezone_type():
